  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      if (paymentResponse.code === '00' && paymentResponse.paymentUrl) {
        ...
        // Mở browser VNPay - user sẽ nhập OTP và thanh toán
        await WebBrowser.openBrowserAsync(paymentResponse.paymentUrl);
        
        // Sau khi user thanh toán:
        // 1. VNPay gọi backend callback (IPN)
        // 2. Backend xử lý và cập nhật DB
        // 3. Backend trả về HTML có Deep Link: quanlyphongtro://payment-callback?invoiceId=X&responseCode=XX
        // 4. App.tsx bắt Deep Link và hiển thị thông báo
        
        // Đóng modal ngay sau khi mở browser
        setIsLoading(false);
        onClose();
      } else {
        setIsLoading(false);
