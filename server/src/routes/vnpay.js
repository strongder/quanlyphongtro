...
function buildSecureHash(params) {
  ...
  const secureHash = crypto
    ...
    .digest("hex");
  return { sortedParams, signData, secureHash };
}
...
router.get("/status/:invoiceId", authRequired, (req, res) => {
  ...
  const payment = db
    .prepare(
      "SELECT * FROM Payment WHERE invoiceId = ? AND paymentMethod = 'VNPAY' ORDER BY createdAt DESC LIMIT 1"
    )
    .get(invoiceId);
...
router.get("/callback", (req, res) => {
  ...
  try {
    ...
    if (existingPayment) {
      ...
      db.prepare(
        `INSERT INTO Payment (invoiceId, tenantId, transactionId, amount, status, responseCode, paymentMethod, paidAt) VALUES (?, ?, ?, ?, ?, ?, 'VNPAY', datetime('now'))`
      ).run(invoiceId, invoice.tenantId, transactionId, vnp_Params.vnp_Amount / 100, paymentStatus, responseCode);
    }
